...
type RequestCallback = (state: 'attempt' | 'request' | 'success' | 'error', proxy: string, proxyId: number) => any
...
export default async function fetchCORS(
    ...
    while (currentProxyIdx < proxies.length) {
        ...
        //
        try {
            if (currentProxyCallback) currentProxyCallback('attempt', proxy, currentProxyIdx)

            const response = await fetch(currUrl, opts)

            if (currentProxyCallback) currentProxyCallback('request', proxy, currentProxyIdx)
            ...
            if (response.ok) {
                ...
                if (currentProxyCallback) currentProxyCallback('success', proxy, currentProxyIdx)
            ...
            if (currentProxyCallback) currentProxyCallback('error', proxy, currentProxyIdx)
        } finally {
            ++currentProxyIdx
        }
    }
