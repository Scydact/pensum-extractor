...
function updateGradeProgress() {
...

    {
        ...
        (createElement(sel, 'option', 'Mostrar todas') as HTMLOptionElement).value = 'noFilter';
        (createElement(sel, 'option', 'Esconder aprobadas') as HTMLOptionElement).value = 'hidePassed';
        (createElement(sel, 'option', 'Solo mostrar aprobadas') as HTMLOptionElement).value = 'showPassed';
...
function createNewPensumTable(data) {
    ...
    for (let x of ['Ct', '✔', 'Codigo', 'Asignatura', 'Créditos', 'Pre-requisitos']) {
...
function saveToLocalStorage() {
    let out = {
        saveVer: saveVer,
        currentCodeAtInputForm: (document.getElementById('codigoMateria') as HTMLInputElement).value,
...
function loadFromLocalStorage() {
    ...
    (document.getElementById('codigoMateria') as HTMLInputElement).value = out.currentCodeAtInputForm;
 ...
 */
async function fetchHtmlAsText(url, opts : any = {}, forceProxy = -1, currentProxyCallback = null) {
    const corsOverride = [
    ...
    while (i < corsOverride.length) {
        ...
        try {
            ...
            if (currentProxyCallback) currentProxyCallback('request', currProxy, i);
            ...
            if (response.ok) {
                ...
                if (currentProxyCallback) currentProxyCallback('success', currProxy, i);
            ...
            if (currentProxyCallback) currentProxyCallback('error', currProxy, i);
        } finally {
            ++i;
        }
    }
...
async function loadPensum() {
    ...
    let codigoMateriaInput = document.getElementById('codigoMateria') as HTMLInputElement;
    currentPensumCode = codigoMateriaInput.value.toUpperCase();

    // helper functions
    const clearInfoWrap = () => {infoWrap.innerHTML = '';}
    const setInfoWrap = (str) => {infoWrap.innerHTML = str;}
