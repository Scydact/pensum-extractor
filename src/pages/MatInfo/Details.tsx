...
export default function Details() {
    ...
    const title = (
        <>
            <MatCode data={mat.code} /> <span>{mat.name}</span>
        </>
    )
    ...
    const reqInfo = prereqs.length > 0 && (
        <Container>
            <Row>Pre-requisitos:</Row>
            <Row>
                {prereqs.map((req, idx) => (
                    <LongMatCode key={idx} data={req} onClick={reqCbFactory(req)} fromMat={mat.code} />
                ))}
            </Row>
        </Container>
    )
    ...
    const coreqInfo = coreqs.length > 0 && (
        <Container>
    ...
    const postreqInfo = postreqs.length > 0 && (
        <Container>
            <Row>Es requisito de:</Row>
            <Row>
                {postreqs.map((req, idx) => (
                    <LongMatCode key={idx} data={req} onClick={reqCbFactory(req)} />
                ))}
            </Row>
        </Container>
    )
...
function Invalid({ code }: { code: string }) {
    return (
        <>
            ...
            <Modal.Body>Intente usando mayusculas o guiones de por medio.</Modal.Body>
        </>
    )
}
