...
type RenderType = Record<
    string,
    (
        contentSize: { width: number; height: number },
        item: MatOrgChartNode,
        pos: { x: number; y: number; width: number; height: number },
        doc: typeof PDFDocumentType,
    ) => any
>
...
const render: RenderType = {
    container(contentSize, item, pos, doc) {
        ...
        doc.roundedRect(pos.x, pos.y, pos.width, pos.height, 5).fill(statusColor)
    ...
    credits(contentSize, item, pos, doc) {
        ...
        doc.polygon([pos.x + pos.width - 30, pos.y], [pos.x + pos.width, pos.y], [pos.x + pos.width, pos.y + 30]).fill(
            (CR_COLORS as any)[item.cr] || CR_COLORS.default,
        )

        doc.fontSize(12)
            .fillColor('white')
            .font('Helvetica')
            .text(item.cr, pos.x + pos.width - 12, pos.y + 7, {
                ellipsis: false,
                width: 10,
                height: 1,
                align: 'right',
    ...
    code(contentSize, item, pos, doc) {
        doc.fontSize(12)
            .fillColor('black')
            .font('Helvetica')
            .text(`[${item.code}]`, pos.x + 7, pos.y + 7, {
                ellipsis: false,
    ...
    name(contentSize, item, pos, doc) {
        doc.fontSize(14)
            .fillColor('black')
            .font('Helvetica')
            .text(item.name, pos.x + 7, pos.y + 7 + 16 + 5, {
                ellipsis: false,
                width: contentSize.width - 7 - 7,
                height: 60,
                align: 'center',
    ...
    period(contentSize, item, pos, doc) {
        doc.fontSize(12)
            .fillColor('darkgray')
            .font('Helvetica')
            .text(item.period, pos.x + 7, pos.y + 100 - 12 - 7, {
                ellipsis: false,
                width: contentSize.width - 7 - 7,
                height: 60,
                align: 'center',
