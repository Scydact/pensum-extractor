...
function parseInfoLine(str: string) {
    const splitOnFirstColon = [str.substring(0, str.indexOf(': ')), str.substring(str.indexOf(': ') + 2)]

    // No colon == just plain text. (x)
    if (splitOnFirstColon[0] === '') return { type: 'simple', data: str } as { type: 'simple'; data: string }

    const splitOnDots = splitOnFirstColon[1].split('. ')
