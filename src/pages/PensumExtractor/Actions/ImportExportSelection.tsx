...
const MatColor = ({ children, style, ...rest }: any) => (
    <span style={{ ...style, color: 'var(--mat-fg-color, inherit)' }} {...rest}>
        {children}
    </span>
)

export function PortPensumSelectModalBtn({
    children,
    tooltip,
    ...rest
}: ButtonProps & { children: any; tooltip: any }) {
...
function PortPensumSelectModal(props: any) {
    return (
        <GenericModal {...props} title="Portar progreso">
            <p>
                Las materias <MatColor className="passed">aprobadas</MatColor> y{' '}
                <MatColor className="course">en curso </MatColor>
                seleccionadas se guardan localmente en la cache del navegador. Al estar guardados en la cache, estos
                datos podrian borrarse espontaneamente.
            </p>
            <p>
                Para evitar la perdida de estos datos, se recomienda exportar la seleccion como un archivo
                (progreso.json).
            </p>
...
const Btns = {
    ...
    TrackerImport: () => {
        ...
        const onClick = () => {
            try {
                upload('.json').then((txt) => {
                    if (typeof txt !== 'string') throw new Error('Error al leer archivo!')
        ...
        return (
            <Button onClick={onClick}>
                <AiOutlineCloudUpload /> Importar selección
            </Button>
        )
    },
    ...
    Excel: () => {
        ...
        return (
            <Button onClick={onClick} disabled={!pensum}>
