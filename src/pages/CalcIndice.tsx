...
export default function CalcIndice() {
    ...
    const formRows = g.formValues.map((f) => (
        <tr key={f.code}>
            <td className={centerY}>
                <MatCode data={f.code} className="me-2" onClick={() => navigate(`/mat/${f.code}`)} />
                {f.name}
            </td>
    ...
    const validOut =
        g.formValues.length === 0 ? (
            <span>
                Para usar esta calculadora, primero debe seleccionar alguna materia como{' '}
                <span className="course" style={{ color: 'var(--mat-fg-color, inherit)' }}>
                    Cursando
                </span>
                .
            </span>
            ...
            <div className="d-flex flex-column gap-3">
                ...
                <Container>
                    <InputGroup>
                        ...
                        <FormControl type="number" disabled className={numFormCl} value={g.periodGPA.toPrecision(5)} />
                    </InputGroup>
                </Container>
    ...
    return <GenericModalNavBack title="Calculadora de indice">{validOut}</GenericModalNavBack>
}
...
const useFormValues = () => {
    ...
    const updateFormValues = useCallback(
        (mats: Pensum.Mat[], prev: FormValue[]) =>
            mats.map((mat) => {
    ...
    useEffect(() => {
        const trackedMats = [...course].map((code) => codeMap.get(code)).filter(Boolean) as Pensum.Mat[]

        setFormValues((prev) => updateFormValues(trackedMats, prev))
    }, [course, codeMap, updateFormValues])

    const passedCr = [...passed].map((code) => codeMap.get(code)?.cr ?? 0).reduce((cum, x) => x + cum, 0)
...
function getPeriodPGA(arr: FormValue[]) {
    const { total, weightSum } = arr.reduce(
        (cum, x) => {
            cum.total += x.weight
...
const InfoGetGPAFromBanner = memo(() => {
    ...
    return (
        <Container>
            <ol>
                <li>
                    Entrar a <OutLink href="https://landing.unapec.edu.do/banner/">Banner</OutLink>.
                </li>
                <li>
                    En "Servicios academicos", ingresar a{' '}
                    <OutLink href="https://sso.unapec.edu.do/ssomanager/c/SSB?pkg=bwskotrn.P_ViewTermTran">
                        "Histórico académico"
                    </OutLink>
                    .
                </li>
                <li>
                    Se le presenta un historial de todas las notas de todas las materias en todos los cuatrimestres. Una
                    vez aquí, se debe bajar hasta que se encuentre la tabla "TOTALES DE HISTÓRICO ACADÉMICO (GRADO)"
                    (antes de la tabla "CURSOS EN PROGRESO").
                </li>
            </ol>
            <ul>
                <li>El valor de "Horas PGA" es la cantidad de creditos acumulados.</li>
                <li>
                    Para conseguir el indice acumulado exacto, dividir los "Puntos de Calidad" entre las "Horas PGA".
                </li>
            </ul>
        </Container>
    )
