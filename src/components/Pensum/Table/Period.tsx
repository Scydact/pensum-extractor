...
export const Period = ({ period, periodLabel, periodNum, cumlen = 0 }: PeriodProps) => {
    ...
    const commonTracker = matSelectHelpers.getCommonTracker(
        tracker,
        period.map((mat) => mat.code),
    )
    ...
    const matrows = filteredPeriod.map((mat, i) => <MatRow key={mat.code} mat={mat} idx={i + cumlen} />)
